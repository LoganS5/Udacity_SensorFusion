...
while True:
    try:
        ...
        # my_lidar_bev = pcl.bev_from_pcl(lidar_pcl, configs_det)
        # cv2.imshow('bev', np.moveaxis(np.array(lidar_bev[0,:,:,:]),0,2))
        # cv2.imshow('my_bev', np.moveaxis(np.array(my_lidar_bev[0,:,:,:]),0,2))
