...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        pos_sens = (self.veh_to_sens*np.vstack([x[0:3,:], [1]]))[0:3,:]
        angle = np.arctan(pos_sens[1]/pos_sens[0])

        return (angle > self.fov[0]) & (angle < self.fov[1])
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            try:
                ...
                hx[0,0] = self.c_i - self.f_i * pos_sens[1] / 0.01
                hx[1,0] = self.c_j - self.f_j * pos_sens[2] / 0.01
